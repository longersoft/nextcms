...
(function ($) {
    $.monte = function (selector, options) {
        ...
        function setStack() {
            $top = $(stack[0]).css(zIndex, 222).show();
            $prev = $(stack[stack.length - 1]).css(cssPrev).css(zIndex, 111).show();
            $next = $(stack[1]).css(cssNext).css(zIndex, 111).show();
        }
        ...
        */
        ...
        function button(className, text) {
            if(className === 'montePrev'){
                return $('<a href="#"/>').addClass('montePrev').text(text).css(zIndex, 333).css("left",($frame.width() - width) / 2);
            }
            if(className === 'monteNext'){
                return $('<a href="#"/>').addClass('monteNext').text(text).css(zIndex, 333).css("right",($frame.width() - width) / 2);
            }else{
                return $('<a href="#"/>').addClass(className).text(text).css(zIndex, 333);
            }
        }
    ...
    //$('.montePrev').css("left",($frame.width() - width) / 2);
